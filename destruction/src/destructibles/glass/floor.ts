 ...
 * downward Rapier ray against static colliders only, cached on a 0.25 m grid. Dice and fallen glass
...
export class FloorProbe {
  ...
  heightAt(x: number, z: number, fromY: number): number {
    const cx = Math.round(x / CELL), cz = Math.round(z / CELL), cy = Math.round(fromY / 0.5);
    ...
    let y = 0;
    try {
      _o.set(cx * CELL, cy * 0.5 + 0.05, cz * CELL);
